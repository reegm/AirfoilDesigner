import numpy as np
from core import config
# from core.solver.error_functions import calculate_euclidean_error, calculate_orthogonal_error  # Deprecated
from core.solver.error_functions import calculate_single_bezier_fitting_error
from utils.bezier_utils import leading_edge_curvature
from core.solver.solver_helpers import (
    get_fixed_inner_x_partition,
    build_control_points_with_fixed,
    smoothness_penalty,
    extract_free_y_from_ctrl,
    make_build_ctrl_fn,
    make_residuals_fn,
    run_minmax_stage,
    get_initial_guess_inner_y,
    g2_constraint_factory_fixed_x,
    minimize_with_debug_with_abort
)
from utils.bezier_utils import leading_edge_curvature
from core.solver.bezier_optimizer import build_bezier_single_fixed_x_msr

def build_bezier_coupled_fixed_x_msr(
    original_upper_data,
    original_lower_data,
    regularization_weight,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    error_function="euclidean",
    logger_func=None,
    abort_flag=None,
):
    """
    Coupled fixed-x Bezier optimizer (G2 at LE, tangency at TE, MSR objective).
    The pre-trailing-edge control point is fixed and not optimized for both surfaces.
    TE tangency constraints are not needed and are removed.
    """
    if logger_func:
        logger_func("Running coupled fixed-x MSR optimization...")
    num_control_points = config.NUM_CONTROL_POINTS_SINGLE_BEZIER
    # Partition for upper and lower
    te_y_upper = float(original_upper_data[-1, 1])
    te_y_lower = float(original_lower_data[-1, 1])
    inner_x_upper, free_idx_u, fixed_idx_u, fixed_y_u = get_fixed_inner_x_partition(
        True, num_control_points, original_upper_data, te_tangent_vector_upper, te_y_upper)
    inner_x_lower, free_idx_l, fixed_idx_l, fixed_y_l = get_fixed_inner_x_partition(
        False, num_control_points, original_lower_data, te_tangent_vector_lower, te_y_lower)
    n_free_u = len(free_idx_u)
    n_free_l = len(free_idx_l)
    init_y_upper_full = get_initial_guess_inner_y(original_upper_data, inner_x_upper)
    init_y_lower_full = get_initial_guess_inner_y(original_lower_data, inner_x_lower)
    init_y_upper = init_y_upper_full[free_idx_u]
    init_y_lower = init_y_lower_full[free_idx_l]
    initial_guess = np.concatenate([init_y_upper, init_y_lower])
    if error_function == "orthogonal":
        def error_func(data, ctrl):
            return calculate_single_bezier_fitting_error(ctrl, data, error_function="orthogonal")
    else:
        def error_func(data, ctrl):
            return calculate_single_bezier_fitting_error(ctrl, data, error_function="euclidean")
    def assemble_polygons(var_y):
        y_u = var_y[:n_free_u]
        y_l = var_y[n_free_u:]
        ctrl_upper = build_control_points_with_fixed(y_u, inner_x_upper, float(original_upper_data[-1, 1]), free_idx_u, fixed_idx_u, fixed_y_u)
        ctrl_lower = build_control_points_with_fixed(y_l, inner_x_lower, float(original_lower_data[-1, 1]), free_idx_l, fixed_idx_l, fixed_y_l)
        return ctrl_upper, ctrl_lower
    def objective(var_y):
        ctrl_u, ctrl_l = assemble_polygons(var_y)
        err_u = error_func(original_upper_data, ctrl_u)
        err_l = error_func(original_lower_data, ctrl_l)
        if isinstance(err_u, tuple): err_u = err_u[0]
        if isinstance(err_l, tuple): err_l = err_l[0]
        smooth = smoothness_penalty(ctrl_u) + smoothness_penalty(ctrl_l)
        return err_u + err_l + regularization_weight * smooth
    
    # Attach debug functions for logging
    if config.DEBUG_WORKER_LOGGING:
        def get_residuals_with_debug(var_y):
            ctrl_u, ctrl_l = assemble_polygons(var_y)
            err_u = error_func(original_upper_data, ctrl_u)
            err_l = error_func(original_lower_data, ctrl_l)
            if isinstance(err_u, tuple): err_u = err_u[0]
            if isinstance(err_l, tuple): err_l = err_l[0]
            # For MSR optimization, we return scalar errors, not residual arrays
            return np.array([err_u, err_l])
        
        objective.__get_residuals__ = get_residuals_with_debug
        objective.__get_max_error__ = lambda var_y: np.max(np.abs(get_residuals_with_debug(var_y)))
        objective.__build_ctrl__ = assemble_polygons
    
    constraints = []  # TE tangency constraints removed
    constraints.append({"type": "eq", "fun": g2_constraint_factory_fixed_x(
        inner_x_upper, free_idx_u, fixed_idx_u, fixed_y_u, te_y_upper,
        inner_x_lower, free_idx_l, fixed_idx_l, fixed_y_l, te_y_lower,
        original_upper_data, original_lower_data)})
    result, _ = minimize_with_debug_with_abort(
        fun=objective,
        x0=initial_guess,
        method='SLSQP',
        constraints=constraints,
        options=config.SLSQP_OPTIONS,
        abort_flag=abort_flag
    )
    if not result.success and logger_func:
        logger_func(f"Coupled fixed-x MSR optimization failed. Using initial guess. Reason: {result.message}")
        var_y_final = initial_guess
    else:
        if logger_func:
            logger_func("Coupled fixed-x MSR optimization succeeded.")
        var_y_final = result.x
    ctrl_upper_final, ctrl_lower_final = assemble_polygons(var_y_final)
    return ctrl_upper_final, ctrl_lower_final

def build_bezier_coupled_fixed_x_minmax(
    original_upper_data,
    original_lower_data,
    regularization_weight,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    error_function="euclidean",
    logger_func=None,
    abort_flag=None,
):
    """
    Coupled fixed-x Bezier optimizer (G2 at LE, tangency at TE, minmax objective).
    The pre-trailing-edge control point is fixed and not optimized for both surfaces.
    TE tangency constraints are not needed and are removed.
    """
    if logger_func:
        logger_func("Stage 1: Running uncoupled fixed-x MSR optimization for initial guess...")
    num_control_points = config.NUM_CONTROL_POINTS_SINGLE_BEZIER
    # Stage 1: Use build_single_bezier_msr for both upper and lower surfaces
    initial_guess = build_coupled_bezier_fixed_x_msr(
        original_upper_data,
        original_lower_data,
        regularization_weight=0,
        te_tangent_vector_upper=te_tangent_vector_upper,
        te_tangent_vector_lower=te_tangent_vector_lower,
        error_function="euclidean",
        logger_func=logger_func,
        abort_flag=abort_flag,
    )
    
    if logger_func:
        logger_func("Stage 2: Running coupled fixed-x minmax optimization...")
    # Extract free y variables for minmax stage
    te_y_upper = float(original_upper_data[-1, 1])
    te_y_lower = float(original_lower_data[-1, 1])
    inner_x_upper, free_idx_u, fixed_idx_u, fixed_y_u = get_fixed_inner_x_partition(
        True, num_control_points, original_upper_data, te_tangent_vector_upper, te_y_upper)
    inner_x_lower, free_idx_l, fixed_idx_l, fixed_y_l = get_fixed_inner_x_partition(
        False, num_control_points, original_lower_data, te_tangent_vector_lower, te_y_lower)
    msr_y_upper = extract_free_y_from_ctrl(initial_guess[0], free_idx_u)
    msr_y_lower = extract_free_y_from_ctrl(initial_guess[1], free_idx_l)
    msr_y = np.concatenate([msr_y_upper, msr_y_lower])
    n_free_u = len(free_idx_u)
    n_free_l = len(free_idx_l)
    def build_ctrls(y):
        y_u = y[:n_free_u]
        y_l = y[n_free_u:]
        ctrl_upper = build_control_points_with_fixed(y_u, inner_x_upper, te_y_upper, free_idx_u, fixed_idx_u, fixed_y_u)
        ctrl_lower = build_control_points_with_fixed(y_l, inner_x_lower, te_y_lower, free_idx_l, fixed_idx_l, fixed_y_l)
        return ctrl_upper, ctrl_lower
    def smooth_coupled(ctrls):
        ctrl_u, ctrl_l = ctrls
        return smoothness_penalty(ctrl_u) + smoothness_penalty(ctrl_l)
    def residuals_fn_coupled(ctrls):
        ctrl_u, ctrl_l = ctrls
        efun = error_function
        if not efun.endswith('_minmax'):
            efun = efun + '_minmax'
        res_u = make_residuals_fn(original_upper_data, error_function, minmax=True)(ctrl_u)
        res_l = make_residuals_fn(original_lower_data, error_function, minmax=True)(ctrl_l)
        return np.concatenate([res_u, res_l])
    def g2_constraint(y):
        ctrls = build_ctrls(y)
        from utils.bezier_utils import leading_edge_curvature
        return leading_edge_curvature(ctrls[0]) + leading_edge_curvature(ctrls[1])
    constraints_fns = [("eq", g2_constraint)]
    control_polys = run_minmax_stage(
        initial_y=msr_y,
        build_control_points_fn=build_ctrls,
        residuals_fn=residuals_fn_coupled,
        regularization_weight=regularization_weight,
        smoothness_fn=smooth_coupled,
        constraints_fns=constraints_fns,
        logger_func=logger_func,
        abort_flag=abort_flag
    )
    if logger_func:
        logger_func("Stage 2: Coupled fixed-x minmax optimization completed.")
    return control_polys

def build_bezier_coupled_free_x_msr(
    original_upper_data,
    original_lower_data,
    regularization_weight,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    error_function="euclidean",
    logger_func=None,
    abort_flag=None,
):
    """
    Coupled free-x Bezier optimizer (G2 at LE, tangency at TE, MSR objective).
    Only the y-coordinates of the inner control points are optimized; x-coordinates are set by variable_x_control_points.
    The initial guess for the pre-TE y is set using the TE vector.
    """
    from utils.control_point_utils import variable_x_control_points
    if logger_func:
        logger_func("Running coupled free-x MSR optimization...")
    paper_x_upper = variable_x_control_points(original_upper_data, config.NUM_CONTROL_POINTS_SINGLE_BEZIER)
    paper_x_lower = variable_x_control_points(original_lower_data, config.NUM_CONTROL_POINTS_SINGLE_BEZIER)
    inner_x_upper = paper_x_upper[1:-1]
    inner_x_lower = paper_x_lower[1:-1]
    n_inner = len(inner_x_upper)
    te_y_upper = float(original_upper_data[-1, 1])
    te_y_lower = float(original_lower_data[-1, 1])
    init_y_upper = np.interp(inner_x_upper, original_upper_data[:, 0], original_upper_data[:, 1])
    init_y_lower = np.interp(inner_x_lower, original_lower_data[:, 0], original_lower_data[:, 1])
    # Set pre-TE y using TE vector
    x_n_minus_1_u = inner_x_upper[-1]
    x_te = 1.0
    tx_te_u, ty_te_u = te_tangent_vector_upper
    if abs(tx_te_u) >= 1e-12:
        init_y_upper[-1] = te_y_upper - (x_te - x_n_minus_1_u) * (ty_te_u / tx_te_u)
    x_n_minus_1_l = inner_x_lower[-1]
    tx_te_l, ty_te_l = te_tangent_vector_lower
    if abs(tx_te_l) >= 1e-12:
        init_y_lower[-1] = te_y_lower - (x_te - x_n_minus_1_l) * (ty_te_l / tx_te_l)
    initial_guess = np.concatenate([init_y_upper, init_y_lower])
    def assemble_polygons(var_y):
        y_u = var_y[:n_inner]
        y_l = var_y[n_inner:]
        ctrl_upper = np.zeros((n_inner + 2, 2))
        ctrl_lower = np.zeros((n_inner + 2, 2))
        ctrl_upper[0] = [0.0, 0.0]
        ctrl_upper[1:-1, 0] = inner_x_upper
        ctrl_upper[1:-1, 1] = y_u
        ctrl_upper[-1] = [1.0, float(original_upper_data[-1, 1])]
        ctrl_lower[0] = [0.0, 0.0]
        ctrl_lower[1:-1, 0] = inner_x_lower
        ctrl_lower[1:-1, 1] = y_l
        ctrl_lower[-1] = [1.0, float(original_lower_data[-1, 1])]
        return ctrl_upper, ctrl_lower
    if error_function == "orthogonal":
        def error_func(data, ctrl):
            return calculate_single_bezier_fitting_error(ctrl, data, error_function="orthogonal")
    else:
        def error_func(data, ctrl):
            return calculate_single_bezier_fitting_error(ctrl, data, error_function="euclidean")
    def objective(var_y):
        ctrl_u, ctrl_l = assemble_polygons(var_y)
        err_u = error_func(original_upper_data, ctrl_u)
        err_l = error_func(original_lower_data, ctrl_l)
        if isinstance(err_u, tuple): err_u = err_u[0]
        if isinstance(err_l, tuple): err_l = err_l[0]
        def _smooth(ctrl):
            if len(ctrl) <= 2: return 0.0
            return np.sum(np.diff(ctrl[:, 1], n=2) ** 2)
        smooth = _smooth(ctrl_u) + _smooth(ctrl_l)
        return err_u + err_l + regularization_weight * smooth
    
    # Attach debug functions for logging
    if config.DEBUG_WORKER_LOGGING:
        def get_residuals_with_debug(var_y):
            ctrl_u, ctrl_l = assemble_polygons(var_y)
            err_u = error_func(original_upper_data, ctrl_u)
            err_l = error_func(original_lower_data, ctrl_l)
            if isinstance(err_u, tuple): err_u = err_u[0]
            if isinstance(err_l, tuple): err_l = err_l[0]
            # For MSR optimization, we return scalar errors, not residual arrays
            return np.array([err_u, err_l])
        
        objective.__get_residuals__ = get_residuals_with_debug
        objective.__get_max_error__ = lambda var_y: np.max(np.abs(get_residuals_with_debug(var_y)))
        objective.__build_ctrl__ = assemble_polygons
    
    constraints = []
    tx_u, ty_u = te_tangent_vector_upper
    px_n_u, py_n_u = 1.0, 0.0
    px_n1_u = inner_x_upper[-1]
    if not np.isclose(tx_u, 0.0):
        def te_tan_upper(var_y):
            y_nm1 = var_y[n_inner - 1]
            return y_nm1 * tx_u - (py_n_u * tx_u - (px_n_u - px_n1_u) * ty_u)
        constraints.append({"type": "eq", "fun": te_tan_upper})
    tx_l, ty_l = te_tangent_vector_lower
    px_n_l, py_n_l = 1.0, 0.0
    px_n1_l = inner_x_lower[-1]
    if not np.isclose(tx_l, 0.0):
        def te_tan_lower(var_y):
            y_nm1_l = var_y[-1]
            return y_nm1_l * tx_l - (py_n_l * tx_l - (px_n_l - px_n1_l) * ty_l)
        constraints.append({"type": "eq", "fun": te_tan_lower})
    def g2_constraint(var_y):
        ctrl_u, ctrl_l = assemble_polygons(var_y)
        return leading_edge_curvature(ctrl_u) + leading_edge_curvature(ctrl_l)
    constraints.append({"type": "eq", "fun": g2_constraint})
    result, _ = minimize_with_debug_with_abort(
        fun=objective,
        x0=initial_guess,
        method='SLSQP',
        constraints=constraints,
        options=config.SLSQP_OPTIONS,
        abort_flag=abort_flag
    )
    if not result.success and logger_func:
        logger_func(f"Coupled free-x MSR optimization failed. Using initial guess. Reason: {result.message}")
        var_y_final = initial_guess
    else:
        if logger_func:
            logger_func("Coupled free-x MSR optimization succeeded.")
        var_y_final = result.x
    ctrl_upper_final, ctrl_lower_final = assemble_polygons(var_y_final)
    return ctrl_upper_final, ctrl_lower_final 

def build_bezier_coupled_free_x_minmax(
    original_upper_data,
    original_lower_data,
    regularization_weight,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    error_function="euclidean",
    logger_func=None,
    abort_flag=None,
):
    """
    Coupled free-x Bezier optimizer (G2 at LE, tangency at TE, minmax objective).
    Two-stage: MSR for initial guess, then true minmax optimization.
    Only the y-coordinates of the inner control points are optimized; x-coordinates are set by variable_x_control_points.
    The initial guess for the pre-TE y is set using the TE vector.
    """
    from utils.control_point_utils import variable_x_control_points
    num_control_points = config.NUM_CONTROL_POINTS_SINGLE_BEZIER
    if logger_func:
        logger_func("Stage 1: Running uncoupled fixed-x MSR optimization for initial guess...")
    # Stage 1: Use build_coupled_bezier_msr for both upper and lower surfaces
    initial_guess = build_coupled_bezier_fixed_x_msr(
        original_upper_data,
        original_lower_data,
        regularization_weight=0,
        te_tangent_vector_upper=te_tangent_vector_upper,
        te_tangent_vector_lower=te_tangent_vector_lower,
        error_function="euclidean",
        logger_func=logger_func,
        abort_flag=abort_flag,
    )
    if logger_func:
        logger_func("Stage 2: Running coupled free-x minmax optimization...")
    # Extract free y variables for minmax stage
    paper_x_upper = variable_x_control_points(original_upper_data, num_control_points)
    paper_x_lower = variable_x_control_points(original_lower_data, num_control_points)
    inner_x_upper = paper_x_upper[1:-1]
    inner_x_lower = paper_x_lower[1:-1]
    n_inner = len(inner_x_upper)
    te_y_upper = float(original_upper_data[-1, 1])
    te_y_lower = float(original_lower_data[-1, 1])
    msr_y_upper = initial_guess[0][1:-1, 1]
    msr_y_lower = initial_guess[1][1:-1, 1]
    msr_y = np.concatenate([msr_y_upper, msr_y_lower])
    def build_ctrls(y):
        y_u = y[:n_inner]
        y_l = y[n_inner:]
        ctrl_upper = np.zeros((n_inner + 2, 2))
        ctrl_lower = np.zeros((n_inner + 2, 2))
        ctrl_upper[0] = [0.0, 0.0]
        ctrl_upper[1:-1, 0] = inner_x_upper
        ctrl_upper[1:-1, 1] = y_u
        ctrl_upper[-1] = [1.0, te_y_upper]
        ctrl_lower[0] = [0.0, 0.0]
        ctrl_lower[1:-1, 0] = inner_x_lower
        ctrl_lower[1:-1, 1] = y_l
        ctrl_lower[-1] = [1.0, te_y_lower]
        return ctrl_upper, ctrl_lower
    def smooth_coupled(ctrls):
        ctrl_u, ctrl_l = ctrls
        return smoothness_penalty(ctrl_u) + smoothness_penalty(ctrl_l)
    def residuals_fn_coupled(ctrls):
        ctrl_u, ctrl_l = ctrls
        efun = error_function
        if not efun.endswith('_minmax'):
            efun = efun + '_minmax'
        res_u = make_residuals_fn(original_upper_data, error_function, minmax=True)(ctrl_u)
        res_l = make_residuals_fn(original_lower_data, error_function, minmax=True)(ctrl_l)
        return np.concatenate([res_u, res_l])
    def g2_constraint(y_aug):
        ctrls = build_ctrls(y_aug[:-1])
        from utils.bezier_utils import leading_edge_curvature
        return leading_edge_curvature(ctrls[0]) + leading_edge_curvature(ctrls[1])
    constraints_fns = [("eq", g2_constraint)]
    control_polys = run_minmax_stage(
        initial_y=msr_y,
        build_control_points_fn=build_ctrls,
        residuals_fn=residuals_fn_coupled,
        regularization_weight=regularization_weight,
        smoothness_fn=smooth_coupled,
        constraints_fns=constraints_fns,
        logger_func=logger_func,
        abort_flag=abort_flag
    )
    if logger_func:
        logger_func("Stage 2: Coupled free-x minmax optimization completed.")
    return control_polys 

def build_bezier_coupled_free_x_minmax_xy(
    original_upper_data,
    original_lower_data,
    regularization_weight,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    error_function="euclidean",
    logger_func=None,
    abort_flag=None,
):
    """
    Coupled free-x Bezier optimizer (G2 at LE, tangency at TE, minmax objective).
    Uses true XY optimization via run_coupled_xy_minmax for both x and y coordinates.
    """
    from utils.control_point_utils import variable_x_control_points


    if logger_func:
        logger_func("Stage 1: Running uncoupled fixed-x MSR optimization for initial guess...")
    
    # Stage 1: Use build_bezier_coupled_fixed_x_msr for initial guess
    initial_guess = build_bezier_coupled_fixed_x_msr(
        original_upper_data,
        original_lower_data,
        regularization_weight=0,
        te_tangent_vector_upper=te_tangent_vector_upper,
        te_tangent_vector_lower=te_tangent_vector_lower,
        error_function="euclidean",
        logger_func=logger_func,
        abort_flag=abort_flag,
    )
    
    if logger_func:
        logger_func("Stage 2: Running coupled free-x minmax optimization (true XY)...")
    
    te_y_upper = float(original_upper_data[-1, 1])
    te_y_lower = float(original_lower_data[-1, 1])
    
    # Use the initial guess from MSR as starting point
    initial_ctrl_upper, initial_ctrl_lower = initial_guess
    
    final_ctrl_upper, final_ctrl_lower = run_coupled_xy_minmax(
        initial_ctrl_upper=initial_ctrl_upper,
        initial_ctrl_lower=initial_ctrl_lower,
        original_upper_data=original_upper_data,
        original_lower_data=original_lower_data,
        error_fn=error_function + "_minmax",
        te_y_upper=te_y_upper,
        te_y_lower=te_y_lower,
        te_tangent_vector_upper=te_tangent_vector_upper,
        te_tangent_vector_lower=te_tangent_vector_lower,
        bounds_x_upper=None,
        bounds_y_upper=None,
        bounds_x_lower=None,
        bounds_y_lower=None,
        regularization_weight=regularization_weight,
        logger_func=logger_func,
        abort_flag=abort_flag
    )
    
    if logger_func:
        logger_func("Stage 2: Coupled free-x minmax optimization (true XY) completed.")
    
    return final_ctrl_upper, final_ctrl_lower 

def build_bezier_coupled_fixed_x_minmax_xy(
    original_upper_data,
    original_lower_data,
    regularization_weight,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    error_function="euclidean",
    logger_func=None,
    abort_flag=None,
):
    """
    Coupled fixed-x Bezier optimizer using xy_minmax approach with softmax objective.
    This provides the benefits of the softmax optimization while maintaining the speed of fixed-x.
    """
    if logger_func:
        logger_func("Running coupled fixed-x xy_minmax optimization...")
    
    num_control_points = config.NUM_CONTROL_POINTS_SINGLE_BEZIER
    te_y_upper = float(original_upper_data[-1, 1])
    te_y_lower = float(original_lower_data[-1, 1])
    
    # Build initial control points for both surfaces
    inner_x_upper, free_idx_u, fixed_idx_u, fixed_y_u = get_fixed_inner_x_partition(
        True, num_control_points, original_upper_data, te_tangent_vector_upper, te_y_upper)
    inner_x_lower, free_idx_l, fixed_idx_l, fixed_y_l = get_fixed_inner_x_partition(
        False, num_control_points, original_lower_data, te_tangent_vector_lower, te_y_lower)
    
    init_y_upper_full = get_initial_guess_inner_y(original_upper_data, inner_x_upper)
    init_y_lower_full = get_initial_guess_inner_y(original_lower_data, inner_x_lower)
    init_y_upper = init_y_upper_full[free_idx_u]
    init_y_lower = init_y_lower_full[free_idx_l]
    
    # Build initial control points
    build_ctrl_upper = make_build_ctrl_fn(inner_x_upper, te_y_upper, free_idx_u, fixed_idx_u, fixed_y_u)
    build_ctrl_lower = make_build_ctrl_fn(inner_x_lower, te_y_lower, free_idx_l, fixed_idx_l, fixed_y_l)
    initial_ctrl_upper = build_ctrl_upper(init_y_upper)
    initial_ctrl_lower = build_ctrl_lower(init_y_lower)
    
    final_ctrl_upper, final_ctrl_lower = run_coupled_fixed_x_xy_minmax(
        initial_ctrl_upper=initial_ctrl_upper,
        initial_ctrl_lower=initial_ctrl_lower,
        original_upper_data=original_upper_data,
        original_lower_data=original_lower_data,
        error_fn=error_function,
        te_y_upper=te_y_upper,
        te_y_lower=te_y_lower,
        te_tangent_vector_upper=te_tangent_vector_upper,
        te_tangent_vector_lower=te_tangent_vector_lower,
        regularization_weight=regularization_weight,
        logger_func=logger_func,
        abort_flag=abort_flag
    )
    
    if logger_func:
        logger_func("Coupled fixed-x xy_minmax optimization completed.")
    return final_ctrl_upper, final_ctrl_lower



def build_bezier_coupled_free_x_msr_xy(
    original_upper_data,
    original_lower_data,
    regularization_weight,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    error_function="euclidean",
    logger_func=None,
    abort_flag=None,
):
    """
    Coupled free-x Bezier optimizer using xy_minmax approach with MSR objective.
    This provides the benefits of xy optimization while using MSR instead of softmax.
    """
    if logger_func:
        logger_func("Running coupled free-x xy MSR optimization...")
    
    num_control_points = config.NUM_CONTROL_POINTS_SINGLE_BEZIER
    te_y_upper = float(original_upper_data[-1, 1])
    te_y_lower = float(original_lower_data[-1, 1])
    
    # Build initial control points for both surfaces
    from utils.control_point_utils import variable_x_control_points
    paper_x_coords_upper = variable_x_control_points(original_upper_data, num_control_points)
    paper_x_coords_lower = variable_x_control_points(original_lower_data, num_control_points)
    
    # Build initial control points
    initial_ctrl_upper = np.zeros((len(paper_x_coords_upper), 2))
    initial_ctrl_upper[:, 0] = paper_x_coords_upper
    initial_ctrl_upper[:, 1] = np.interp(paper_x_coords_upper, original_upper_data[:, 0], original_upper_data[:, 1])
    initial_ctrl_upper[0] = [0.0, 0.0]  # LE
    initial_ctrl_upper[-1] = [1.0, te_y_upper]  # TE
    
    initial_ctrl_lower = np.zeros((len(paper_x_coords_lower), 2))
    initial_ctrl_lower[:, 0] = paper_x_coords_lower
    initial_ctrl_lower[:, 1] = np.interp(paper_x_coords_lower, original_lower_data[:, 0], original_lower_data[:, 1])
    initial_ctrl_lower[0] = [0.0, 0.0]  # LE
    initial_ctrl_lower[-1] = [1.0, te_y_lower]  # TE
    
    final_ctrl_upper, final_ctrl_lower = run_coupled_xy_minmax(
        initial_ctrl_upper=initial_ctrl_upper,
        initial_ctrl_lower=initial_ctrl_lower,
        original_upper_data=original_upper_data,
        original_lower_data=original_lower_data,
        error_fn=error_function,
        te_y_upper=te_y_upper,
        te_y_lower=te_y_lower,
        te_tangent_vector_upper=te_tangent_vector_upper,
        te_tangent_vector_lower=te_tangent_vector_lower,
        regularization_weight=regularization_weight,
        logger_func=logger_func,
        abort_flag=abort_flag
    )
    
    if logger_func:
        logger_func("Coupled free-x xy MSR optimization completed.")
    return final_ctrl_upper, final_ctrl_lower 

# --- XY Optimization Functions (moved from solver_helpers.py) ---

def run_coupled_xy_minmax(
    initial_ctrl_upper,
    initial_ctrl_lower,
    original_upper_data,
    original_lower_data,
    error_fn,
    te_y_upper,
    te_y_lower,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    bounds_x_upper=None,
    bounds_y_upper=None,
    bounds_x_lower=None,
    bounds_y_lower=None,
    regularization_weight=0.0,
    logger_func=None,
    abort_flag=None
):
    from core.solver.error_functions import calculate_single_bezier_fitting_error
    
    n_upper = len(initial_ctrl_upper)
    n_lower = len(initial_ctrl_lower)
    
    # For LE constraint: second control point x is fixed to 0, so we exclude it from variables
    # Remove TE constraint: let second-to-last point move freely
    x0_upper = initial_ctrl_upper[2:-1, 0]  # exclude LE, second point, and TE
    y0_upper = initial_ctrl_upper[1:-1, 1]  # exclude LE and TE, but include second point y
    x0_lower = initial_ctrl_lower[2:-1, 0]  # exclude LE, second point, and TE
    y0_lower = initial_ctrl_lower[1:-1, 1]  # exclude LE and TE, but include second point y
    
    xy0 = np.concatenate([x0_upper, y0_upper, x0_lower, y0_lower])

    # Initial residuals to set t0
    def build_ctrl_upper(xy):
        n_x_vars_upper = n_upper - 3  # number of free x variables (exclude LE, second point, and TE)
        n_y_vars_upper = n_upper - 2  # number of y variables (exclude LE and TE, but include second point y)
        
        x_inner_upper = xy[:n_x_vars_upper]
        y_inner_upper = xy[n_x_vars_upper:n_x_vars_upper + n_y_vars_upper]
        
        ctrl = np.zeros((n_upper, 2))
        ctrl[0] = [0.0, 0.0]  # LE
        ctrl[1] = [0.0, y_inner_upper[0]]  # Second point: x=0, y free
        
        # Remaining inner points (including second-to-last)
        ctrl[2:-1, 0] = x_inner_upper  # Remaining inner x-coordinates
        ctrl[2:-1, 1] = y_inner_upper[1:]  # Remaining inner y-coordinates
        
        ctrl[-1] = [1.0, te_y_upper]  # TE
        return ctrl

    def build_ctrl_lower(xy):
        n_x_vars_upper = n_upper - 3
        n_y_vars_upper = n_upper - 2
        n_x_vars_lower = n_lower - 3
        n_y_vars_lower = n_lower - 2
        
        start_idx_lower = n_x_vars_upper + n_y_vars_upper
        x_inner_lower = xy[start_idx_lower:start_idx_lower + n_x_vars_lower]
        y_inner_lower = xy[start_idx_lower + n_x_vars_lower:]
        
        ctrl = np.zeros((n_lower, 2))
        ctrl[0] = [0.0, 0.0]  # LE
        ctrl[1] = [0.0, y_inner_lower[0]]  # Second point: x=0, y free
        
        # Remaining inner points (including second-to-last)
        ctrl[2:-1, 0] = x_inner_lower  # Remaining inner x-coordinates
        ctrl[2:-1, 1] = y_inner_lower[1:]  # Remaining inner y-coordinates
        
        ctrl[-1] = [1.0, te_y_lower]  # TE
        return ctrl

    def residuals_fn_upper(ctrl):
        residuals, _, _ = calculate_single_bezier_fitting_error(
            ctrl, original_upper_data, error_function=error_fn, return_max_error=False, return_all=True
        )
        return residuals

    def residuals_fn_lower(ctrl):
        residuals, _, _ = calculate_single_bezier_fitting_error(
            ctrl, original_lower_data, error_function=error_fn, return_max_error=False, return_all=True
        )
        return residuals

    res0_upper = residuals_fn_upper(initial_ctrl_upper)
    res0_lower = residuals_fn_lower(initial_ctrl_lower)
    t0 = max(np.max(np.abs(res0_upper)), np.max(np.abs(res0_lower))) * 1.05
    vars0 = xy0

    # Track best configuration during optimization
    best_max_error = float("inf")
    best_xy = None
    
    def full_obj(xy):
        nonlocal best_max_error, best_xy
        
        ctrl_upper = build_ctrl_upper(xy)
        ctrl_lower = build_ctrl_lower(xy)
        
        residuals_upper = residuals_fn_upper(ctrl_upper)
        residuals_lower = residuals_fn_lower(ctrl_lower)
        
        # Combine residuals from both surfaces
        all_residuals = np.concatenate([residuals_upper, residuals_lower])
        
        # Calculate true max error for tracking
        true_max_error = np.max(np.abs(all_residuals))
        
        # Update best configuration if we found a better max error
        if true_max_error < best_max_error:
            best_max_error = true_max_error
            best_xy = np.copy(xy)
        
        # Optimize softmax for numerical stability
        alpha = config.SOFTMAX_ALPHA
        softmax_error = np.log(np.sum(np.exp(alpha * np.abs(all_residuals)))) / alpha
        
        # Add smoothness penalty for both surfaces
        smoothness_penalty_val = (smoothness_penalty(ctrl_upper) + smoothness_penalty(ctrl_lower)) if regularization_weight > 0 else 0.0
        
        # Calculate final objective
        position_weight = 100 if regularization_weight == 0.0 else 1
        position_term = position_weight
        smoothness_term = regularization_weight * smoothness_penalty_val
        total_obj = softmax_error + position_term + smoothness_term
        
        return total_obj
    
    # Attach debug functions for logging
    if config.DEBUG_WORKER_LOGGING:
        def get_residuals_with_debug(xy):
            ctrl_upper = build_ctrl_upper(xy)
            ctrl_lower = build_ctrl_lower(xy)
            res_upper = residuals_fn_upper(ctrl_upper)
            res_lower = residuals_fn_lower(ctrl_lower)
            return np.concatenate([res_upper, res_lower])
        
        full_obj.__get_residuals__ = get_residuals_with_debug
        full_obj.__get_max_error__ = lambda xy: np.max(np.abs(get_residuals_with_debug(xy)))
        full_obj.__build_ctrl__ = build_ctrl_upper  # For debug output compatibility
        full_obj.__build_ctrl_upper__ = build_ctrl_upper
        full_obj.__build_ctrl_lower__ = build_ctrl_lower
        full_obj.__te_y__ = te_y_upper  # Use upper surface for debug output
        full_obj.__original_data__ = original_upper_data  # Use upper surface for debug output
        full_obj.__te_y_upper__ = te_y_upper
        full_obj.__te_y_lower__ = te_y_lower
        full_obj.__original_upper_data__ = original_upper_data
        full_obj.__original_lower_data__ = original_lower_data

    constraints = []

    # TE tangent constraints for upper surface
    if te_tangent_vector_upper is not None and te_y_upper is not None:
        def make_te_vector_constraint_upper(te_tangent_vector_upper, te_y_upper, n_x_vars_upper, n_y_vars_upper):
            tx_te, ty_te = te_tangent_vector_upper
            norm = np.sqrt(tx_te**2 + ty_te**2) or 1e-12

            def constraint(xy):
                x_nm1 = xy[n_x_vars_upper - 1]
                y_nm1 = xy[n_x_vars_upper + n_y_vars_upper - 1]
                dx = 1.0 - x_nm1
                dy = te_y_upper - y_nm1
                return (dx * ty_te - dy * tx_te) / norm

            return constraint

        constraints.append({
            "type": "eq",
            "fun": make_te_vector_constraint_upper(te_tangent_vector_upper, te_y_upper, n_upper - 3, n_upper - 2)
        })

    # TE tangent constraints for lower surface
    if te_tangent_vector_lower is not None and te_y_lower is not None:
        def make_te_vector_constraint_lower(te_tangent_vector_lower, te_y_lower, n_x_vars_upper, n_y_vars_upper, n_x_vars_lower, n_y_vars_lower):
            tx_te, ty_te = te_tangent_vector_lower
            norm = np.sqrt(tx_te**2 + ty_te**2) or 1e-12

            def constraint(xy):
                start_idx_lower = n_x_vars_upper + n_y_vars_upper
                x_nm1 = xy[start_idx_lower + n_x_vars_lower - 1]
                y_nm1 = xy[start_idx_lower + n_x_vars_lower + n_y_vars_lower - 1]
                dx = 1.0 - x_nm1
                dy = te_y_lower - y_nm1
                return (dx * ty_te - dy * tx_te) / norm

            return constraint

        constraints.append({
            "type": "eq",
            "fun": make_te_vector_constraint_lower(te_tangent_vector_lower, te_y_lower, n_upper - 3, n_upper - 2, n_lower - 3, n_lower - 2)
        })

    # G2 constraint at leading edge
    def g2_constraint(xy):
        ctrl_upper = build_ctrl_upper(xy)
        ctrl_lower = build_ctrl_lower(xy)
        return leading_edge_curvature(ctrl_upper) + leading_edge_curvature(ctrl_lower)

    constraints.append({
        "type": "eq",
        "fun": g2_constraint
    })

    # Set bounds
    if bounds_x_upper is None:
        bounds_x_upper = [(0.0, 1.0)] * (n_upper - 3)  # exclude LE, second point, and TE
    if bounds_y_upper is None:
        bounds_y_upper = [(-1.0, 1.0)] * (n_upper - 2)  # exclude LE and TE, but include second point y
    if bounds_x_lower is None:
        bounds_x_lower = [(0.0, 1.0)] * (n_lower - 3)  # exclude LE, second point, and TE
    if bounds_y_lower is None:
        bounds_y_lower = [(-1.0, 1.0)] * (n_lower - 2)  # exclude LE and TE, but include second point y
    
    bounds = bounds_x_upper + bounds_y_upper + bounds_x_lower + bounds_y_lower
    
    success_threshold = config.MAX_ERROR_THRESHOLD
    result, trace = minimize_with_debug_with_abort(
        fun=full_obj,
        x0=vars0,
        method="SLSQP",
        bounds=bounds,
        constraints=constraints,
        options=config.SLSQP_OPTIONS,
        success_threshold=success_threshold,
        abort_flag=abort_flag
    )

    if logger_func:
        if result.success:
            logger_func("Coupled XY minmax optimization succeeded.")
        else:
            logger_func(f"Coupled XY minmax optimization failed: {result.message}")

    # Use the best configuration found during optimization (if available)
    if best_xy is not None:
        final_ctrl_upper = build_ctrl_upper(best_xy)
        final_ctrl_lower = build_ctrl_lower(best_xy)
        if logger_func:
            logger_func(f"Using best configuration found (max error: {best_max_error:.6e})")
    else:
        if result.success:
            final_ctrl_upper = build_ctrl_upper(result.x)
            final_ctrl_lower = build_ctrl_lower(result.x)
        else:
            final_ctrl_upper = initial_ctrl_upper
            final_ctrl_lower = initial_ctrl_lower
    
    return final_ctrl_upper, final_ctrl_lower

def run_coupled_fixed_x_xy_minmax(
    initial_ctrl_upper,
    initial_ctrl_lower,
    original_upper_data,
    original_lower_data,
    error_fn,
    te_y_upper,
    te_y_lower,
    te_tangent_vector_upper,
    te_tangent_vector_lower,
    regularization_weight=0.0,
    logger_func=None,
    abort_flag=None
):
    """
    Coupled fixed-x version of xy_minmax that uses the softmax approach but with fixed x-coordinates.
    This provides the benefits of the softmax optimization while maintaining the speed of fixed-x.
    """
    from core.solver.error_functions import calculate_single_bezier_fitting_error
    
    n_upper = len(initial_ctrl_upper)
    n_lower = len(initial_ctrl_lower)
    
    # For fixed-x, we only optimize y-coordinates (except LE and TE which are fixed)
    y0_upper = initial_ctrl_upper[1:-1, 1]  # exclude LE and TE, but include all inner y-coordinates
    y0_lower = initial_ctrl_lower[1:-1, 1]  # exclude LE and TE, but include all inner y-coordinates
    y0 = np.concatenate([y0_upper, y0_lower])

    def build_ctrl_upper(y):
        ctrl = np.zeros((n_upper, 2))
        ctrl[0] = [0.0, 0.0]  # LE fixed
        
        # Inner points: x-coordinates are fixed, y-coordinates are optimized
        ctrl[1:-1, 0] = initial_ctrl_upper[1:-1, 0]  # Fixed x-coordinates
        ctrl[1:-1, 1] = y  # Optimized y-coordinates
        
        ctrl[-1] = [1.0, te_y_upper]  # TE fixed
        return ctrl

    def build_ctrl_lower(y):
        ctrl = np.zeros((n_lower, 2))
        ctrl[0] = [0.0, 0.0]  # LE fixed
        
        # Inner points: x-coordinates are fixed, y-coordinates are optimized
        ctrl[1:-1, 0] = initial_ctrl_lower[1:-1, 0]  # Fixed x-coordinates
        ctrl[1:-1, 1] = y  # Optimized y-coordinates
        
        ctrl[-1] = [1.0, te_y_lower]  # TE fixed
        return ctrl

    def residuals_fn_upper(ctrl):
        residuals, _, _ = calculate_single_bezier_fitting_error(
            ctrl, original_upper_data, error_function=error_fn, return_max_error=False, return_all=True
        )
        return residuals

    def residuals_fn_lower(ctrl):
        residuals, _, _ = calculate_single_bezier_fitting_error(
            ctrl, original_lower_data, error_function=error_fn, return_max_error=False, return_all=True
        )
        return residuals

    res0_upper = residuals_fn_upper(initial_ctrl_upper)
    res0_lower = residuals_fn_lower(initial_ctrl_lower)
    t0 = max(np.max(np.abs(res0_upper)), np.max(np.abs(res0_lower))) * 1.05

    # Track best configuration during optimization
    best_max_error = float("inf")
    best_y = None
    
    def full_obj(y):
        nonlocal best_max_error, best_y
        
        n_y_upper = len(y0_upper)
        y_upper = y[:n_y_upper]
        y_lower = y[n_y_upper:]
        
        ctrl_upper = build_ctrl_upper(y_upper)
        ctrl_lower = build_ctrl_lower(y_lower)
        
        residuals_upper = residuals_fn_upper(ctrl_upper)
        residuals_lower = residuals_fn_lower(ctrl_lower)
        
        # Calculate true max error for tracking
        true_max_error = max(np.max(np.abs(residuals_upper)), np.max(np.abs(residuals_lower)))
        
        # Update best configuration if we found a better max error
        if true_max_error < best_max_error:
            best_max_error = true_max_error
            best_y = np.copy(y)
        
        # Optimize softmax for numerical stability
        alpha = config.SOFTMAX_ALPHA
        softmax_error_upper = np.log(np.sum(np.exp(alpha * np.abs(residuals_upper)))) / alpha
        softmax_error_lower = np.log(np.sum(np.exp(alpha * np.abs(residuals_lower)))) / alpha
        softmax_error = softmax_error_upper + softmax_error_lower
        
        # Add smoothness penalty
        smoothness_penalty_val = smoothness_penalty(ctrl_upper) + smoothness_penalty(ctrl_lower) if regularization_weight > 0 else 0.0
        
        # Calculate final objective
        position_weight = 100 if regularization_weight == 0.0 else 1
        position_term = position_weight
        smoothness_term = regularization_weight * smoothness_penalty_val
        total_obj = softmax_error + position_term + smoothness_term
        
        return total_obj
    
    # Attach debug functions for logging
    if config.DEBUG_WORKER_LOGGING:
        def get_residuals_with_debug(y):
            n_y_upper = len(y0_upper)
            y_upper = y[:n_y_upper]
            y_lower = y[n_y_upper:]
            ctrl_upper = build_ctrl_upper(y_upper)
            ctrl_lower = build_ctrl_lower(y_lower)
            return np.concatenate([residuals_fn_upper(ctrl_upper), residuals_fn_lower(ctrl_lower)])
        
        full_obj.__get_residuals__ = get_residuals_with_debug
        full_obj.__get_max_error__ = lambda y: np.max(np.abs(get_residuals_with_debug(y)))
        full_obj.__build_ctrl__ = lambda y: (build_ctrl_upper(y[:len(y0_upper)]), build_ctrl_lower(y[len(y0_upper):]))

    constraints = []
    
    # Add G2 constraint at leading edge
    def g2_constraint(y):
        n_y_upper = len(y0_upper)
        y_upper = y[:n_y_upper]
        y_lower = y[n_y_upper:]
        
        ctrl_upper = build_ctrl_upper(y_upper)
        ctrl_lower = build_ctrl_lower(y_lower)
        
        # G2 constraint: second derivatives should be equal at LE
        # For Bezier curves, this means the second control point should be symmetric
        # LE is at (0,0), so second point should be at (0, y) for both surfaces
        # The constraint is that the y-coordinates should be equal in magnitude but opposite in sign
        return ctrl_upper[1, 1] + ctrl_lower[1, 1]  # Should be 0 for G2 continuity
    
    constraints.append({"type": "eq", "fun": g2_constraint})

    result, iteration_data = minimize_with_debug_with_abort(
        fun=full_obj,
        x0=y0,
        method="SLSQP",
        constraints=constraints,
        options=config.SLSQP_OPTIONS,
        success_threshold=config.MAX_ERROR_THRESHOLD,
        abort_flag=abort_flag
    )

    if logger_func:
        if result.success:
            logger_func("Coupled fixed-x softmax optimization succeeded.")
        else:
            logger_func(f"Coupled fixed-x softmax optimization failed: {result.message}")

    # Use the best configuration found during optimization (if available)
    if best_y is not None:
        n_y_upper = len(y0_upper)
        final_ctrl_upper = build_ctrl_upper(best_y[:n_y_upper])
        final_ctrl_lower = build_ctrl_lower(best_y[n_y_upper:])
        if logger_func:
            logger_func(f"Using best configuration found (max error: {best_max_error:.6e})")
    else:
        n_y_upper = len(y0_upper)
        if result.success:
            final_ctrl_upper = build_ctrl_upper(result.x[:n_y_upper])
            final_ctrl_lower = build_ctrl_lower(result.x[n_y_upper:])
        else:
            final_ctrl_upper = build_ctrl_upper(y0_upper)
            final_ctrl_lower = build_ctrl_lower(y0_lower)
    
    return final_ctrl_upper, final_ctrl_lower 